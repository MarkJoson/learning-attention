"""薄适配层（本仓库自写）—— 让拷贝来的 KDA kernel 脱离 fla 包独立运行。

KDA（Kimi Delta Attention）的 kernel 比 DeltaNet 更复杂，依赖更多工具符号。本文件在 11 章
compat 的基础上多提供：
  - exp / gather / softplus —— 数学/门控用的 triton 算子；
  - IS_TF32_SUPPORTED / tensor_cache —— GPU 标志 / memoize（no-op）；
  - **cp stub** —— context-parallel（多卡）的 FLACPContext + 预处理函数。KDA 的 chunk_fwd/bwd 静态
...
class FLACPContext:  # noqa: D401
    """cp 多卡上下文占位（单卡用不到；KDA 仅用它做类型注解 + None 判断）。"""
    pass
...
# chunk_local_cumsum 实为拷贝的 kernel（_fla_kda_cumsum，门控的块内 prefix-sum）。某些 kernel 以
# `from fla.ops.utils import chunk_local_cumsum`（已重写为本 compat）取用，这里 re-export。
# _fla_kda_cumsum 只依赖本 compat 上方已定义的符号（fla_cache_autotune 等），循环 import 安全。
from _fla_kda_cumsum import chunk_local_cumsum  # noqa: E402
