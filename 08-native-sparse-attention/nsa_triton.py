# =============================================================================
# 来源标注 (Provenance) —— 本仓库 08-native-sparse-attention 的 NSA 稀疏 attention triton kernel
# -----------------------------------------------------------------------------
# 本文件的 Triton kernel 完整提取自 lucidrains 的 native-sparse-attention-pytorch，
# **计算逻辑一字未改**，仅去除 1 个库内依赖：
#   原: from native_sparse_attention_pytorch.tensor_typing import Float, Int, Bool
#   → 内联为零依赖占位（Float/Int/Bool 只是 jaxtyping 张量 shape 注解，from __future__
#     import annotations 下不在运行时求值，不参与任何计算）。去依赖后本文件不再 import
#     lucidrains 包本体，仅依赖 torch / triton / einx / einops 即可独立运行。
#
#   source repo : https://github.com/lucidrains/native-sparse-attention-pytorch
#   source file : native_sparse_attention_pytorch/triton_native_sparse_attention.py
#   version     : 0.2.3 (pip)
#   license     : MIT (Copyright Phil Wang / lucidrains)
#   kernel 谱系 : 改编自 Dao-AILab/flash-attention 的 triton flash kernel（见下方第 ~13 行原注释）。
#   algorithm   : DeepSeek NSA（Native Sparse Attention）的 fine-selection + sliding 分支的
#                 硬件对齐稀疏 flash kernel（forward + backward，含 GQA）。
#
# 这 1987 行实现的是 NSA 三分支里**计算最重**的一段：给定每个 query 选中的 top-k 块索引
# selected_block_indices，只对这些块（外加滑窗）做 flash 式在线 softmax。而"块压缩、算块
# 分数、top-k 选块、门控合并"是 host 端 PyTorch 逻辑（见简要版 nsa.py 与 README 拆解）。
#
#   入口: native_sparse_attend(fq, fk, fv, block_size, selected_block_indices, fmask, ...)
#
...
# --- 去依赖（本仓库改动处，不改计算逻辑）-----------------------------------------
# 原: from native_sparse_attention_pytorch.tensor_typing import Float, Int, Bool
# Float/Int/Bool 仅用于 native_sparse_attend 签名的张量 shape 注解；from __future__
# import annotations 下注解不在运行时求值，故下面的零依赖占位不影响任何计算。
class _ShapeAnno:
    def __getitem__(self, _shapes):
        return None
Float = Int = Bool = _ShapeAnno()
# -------------------------------------------------------------------------------
