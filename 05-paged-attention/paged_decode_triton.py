# =============================================================================
# 来源标注 (Provenance) —— 本仓库 05-paged-attention
# -----------------------------------------------------------------------------
# 本文件的 Triton kernel 提取自 lightllm，**计算逻辑未作任何改动**：其依赖本就干净
# （只用 torch/triton/math），无框架内部耦合，无需替换。
#
#   source repo : https://github.com/ModelTC/lightllm
#   source file : lightllm/common/basemodel/triton_kernel/att/decode_att/gqa/
#                 gqa_decode_flashattention_nopad.py
#   commit      : 9ae6a5d4886312bd827295f3cb0de231639f0c77
#   license     : Apache-2.0
#
# 核心机制 —— paged 间接寻址（整份 kernel 的灵魂就这两步）：
...
# 场景：decode（每序列 1 个 query token）、GQA。
