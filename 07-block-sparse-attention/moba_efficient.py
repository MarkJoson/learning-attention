# =============================================================================
# 来源标注 (Provenance) —— 本仓库 07-block-sparse-attention 的 MoBA 高效做法（补充·仅供阅读）
# -----------------------------------------------------------------------------
# 完整拷贝自 Kimi / Moonshot AI 的 MoBA，**计算逻辑一字未改**。
#   source repo : https://github.com/MoonshotAI/MoBA
#   source file : moba/moba_efficient.py
#   commit      : b5d58363311d3ca946f1ec444182727c15e338b5
#   license     : MIT (Copyright © 2025 Moonshot AI)
#
# ⚠️ 依赖 flash-attn==2.6.3；本仓库环境（RTX 4090 + torch 2.13-dev）未安装，故本文件
#    **仅供阅读** MoBA 的"高效做法"，不参与本仓库测试（可跑的是 moba_naive.py）。
#
# 关键认知：MoBA 高效版**没有自己的 triton kernel**。它把动态块稀疏拆成两路——
#   ① self-attn：每个 chunk 内 + 当前块（causal）
#   ② moba-attn：每个 query 选中的历史 chunk，用 varlen 把 (query, chunk) 对**重排成稠密批**
# 两路都交给 flash_attn 的 _flash_attn_varlen_forward 计算，再用**在线 softmax**（见
# MixedAttention 里的 lse 合并）把两路输出合并。这是"借现成 flash kernel + 数据重排实现
# 动态稀疏"的代表性工程路线，与 08 NSA"自写稀疏 triton kernel"形成对照。
