# =============================================================================
# 来源标注 (Provenance) —— 本仓库 11-deltanet（DeltaNet 深度优化版，解耦自 fla）
# -----------------------------------------------------------------------------
# 完整拷贝自 fla-org/flash-linear-attention，**计算逻辑一字未改**，仅把对 fla 框架的 import 改指向
# 本地（kernel 文件 _fla_* / 薄适配层 _fla_dn_compat.py）。关键：用 no-op dispatch 绕过 fla 的后端分派
# （CP / TileLang），使完整依赖闭包从 ~27 个文件收敛到 8 个 triton 文件、脱离 fla 包独立运行。
#   source repo : https://github.com/fla-org/flash-linear-attention
#   source file : fla/ops/delta_rule/chunk.py
#   commit      : 0b27f7b
...
from _fla_l2norm import l2norm_bwd, l2norm_fwd
from _fla_chunk_delta_h import chunk_gated_delta_rule_bwd_dhu, chunk_gated_delta_rule_fwd_h
from _fla_chunk_o import chunk_bwd_dqkwg, chunk_bwd_dv_local, chunk_fwd_o
from _fla_wy_fast import prepare_wy_repr_bwd, prepare_wy_repr_fwd, recompute_w_u_fwd
from _fla_dn_compat import prepare_chunk_indices
from _fla_dn_compat import autocast_custom_bwd, autocast_custom_fwd, input_guard
...
def chunk_delta_rule(
    ...
    **kwargs,
    ...
    Examples::
        ...
        >>> from _fla_dn_compat import chunk_delta_rule
