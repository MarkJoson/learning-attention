# =============================================================================
# 来源标注 (Provenance) —— 本仓库 15-mamba（Mamba2 SSD = fla simple_gla，解耦自 fla）
# -----------------------------------------------------------------------------
# 完整拷贝自 fla-org/flash-linear-attention 的 simple_gla（= Mamba2 的 State Space Duality：
# head-wise 标量衰减门控，对比 GLA 的 per-channel），**计算逻辑一字未改**，仅把对 fla 框架的
# import 改指向本地（_fla_ssd_* / 薄适配层 _fla_ssd_compat.py）。no-op dispatch 绕过后端分派，
# 脱离 fla 包独立运行。
#   source repo : https://github.com/fla-org/flash-linear-attention
#   source file : fla/ops/simple_gla/chunk.py
...
from _fla_ssd_chunk_h import chunk_bwd_dh, chunk_fwd_h
from _fla_ssd_chunk_o import chunk_bwd_dqkwg, chunk_bwd_dv, chunk_fwd_o
from _fla_ssd_compat import chunk_local_cumsum, prepare_chunk_indices
from _fla_ssd_compat import RCP_LN2
from _fla_ssd_compat import autocast_custom_bwd, autocast_custom_fwd, input_guard
...
def chunk_simple_gla(
    ...
    **kwargs,
    ...
    Examples::
        ...
        >>> from _fla_ssd_compat import chunk_simple_gla
